// TMDB Available Regions for Watch Providers
...
const COUNTRY_CODE_TO_NAME = new Map(
  AVAILABLE_REGIONS.map((country) => [country.iso_3166_1, country.english_name])
);

const COUNTRY_CODE_TO_NATIVE_NAME = new Map(
  AVAILABLE_REGIONS.map((country) => [country.iso_3166_1, country.native_name])
);

// Helper functions
export const getCountryName = (countryCode) => {
  return COUNTRY_CODE_TO_NAME.get(countryCode) || countryCode;
};

export const getCountryNativeName = (countryCode) => {
  return COUNTRY_CODE_TO_NATIVE_NAME.get(countryCode) || countryCode;
};

export const isValidCountryCode = (countryCode) => {
  return COUNTRY_CODE_TO_NAME.has(countryCode);
};

// Get countries sorted alphabetically by English name
export const getSortedCountries = () => {
  return [...AVAILABLE_REGIONS].sort((a, b) =>
    a.english_name.localeCompare(b.english_name)
  );
};

// Get popular countries (commonly used ones) at the top
export const getPopularCountries = () => {
  ...
  const popular = popularCodes
    .filter((code) => COUNTRY_CODE_TO_NAME.has(code))
    .map((code) =>
      AVAILABLE_REGIONS.find((country) => country.iso_3166_1 === code)
    )
    .filter(Boolean);

  const remaining = AVAILABLE_REGIONS.filter(
    (country) => !popularCodes.includes(country.iso_3166_1)
...
export const searchCountries = (searchTerm) => {
  if (!searchTerm) return getSortedCountries();

  const term = searchTerm.toLowerCase();
  return AVAILABLE_REGIONS.filter(
    (country) =>
      country.english_name.toLowerCase().includes(term) ||
      country.native_name.toLowerCase().includes(term) ||
      country.iso_3166_1.toLowerCase().includes(term)
  ).sort((a, b) => a.english_name.localeCompare(b.english_name));
};

// TMDB image URL helpers
export const getTMDBImageUrl = (logoPath, size = "original") => {
  if (!logoPath) return null;
  return `https://image.tmdb.org/t/p/${size}${logoPath}`;
};

// Watch provider tab mapping
export const WATCH_PROVIDER_TABS = {
  flatrate: "Stream",
  buy: "Buy",
  rent: "Rent",
};

export const getTabDisplayName = (tabKey) => {
  return WATCH_PROVIDER_TABS[tabKey] || tabKey;
};
