// lib/releaseScheduleCountries.js

// Complete country data from your provided list
const countriesData = [
  {
    iso_3166_1: "AD",
...
const countryMap = new Map(
  countriesData.map((country) => [country.iso_3166_1, country])
);

// Popular countries matching watchproviders order EXACTLY
...
export const getReleaseTypeDisplayName = (releaseType) => {
  const typeNames = {
    premiere: "Premiere",
    theatrical_limited: "Limited",
    theatrical: "Theatrical",
    digital: "Digital",
    physical: "Physical",
    tv: "TV",
  };
  return typeNames[releaseType] || releaseType;
};
...
// Get certification badge color based on rating
export const getCertificationColor = (certification) => {
  if (!certification) return "bg-gray-500";

  const cert = certification.toString().toUpperCase();
...
// Sort releases by date (earliest first)
...
/**
 * Get filtered countries with proper popular/all logic
 * Matches watchproviders behavior exactly
 */
export const getFilteredCountriesWithToggle = (
  availableCountryCodes,
  showAllCountries = false
) => {
  if (!availableCountryCodes || !Array.isArray(availableCountryCodes)) {
    return { countries: [], showToggle: false };
  }

  const uniqueCountries = [...new Set(availableCountryCodes.filter(Boolean))];

  // Get popular countries that exist in available data
  const popularInAvailable = uniqueCountries.filter((code) =>
    POPULAR_COUNTRIES.includes(code)
  );

  // Only show toggle if we have popular countries AND they're not the same as all countries
  const showToggle =
    popularInAvailable.length > 0 &&
    popularInAvailable.length !== uniqueCountries.length;

  // Convert to country objects
  const allCountryObjects = uniqueCountries.map((code) => {
    const countryData = countryMap.get(code);
    return {
      iso_3166_1: code,
      english_name: countryData?.english_name || code,
      native_name:
        countryData?.native_name || countryData?.english_name || code,
      isPopular: POPULAR_COUNTRIES.includes(code),
    };
  });

  // Sort: popular first (in watchproviders order), then others (alphabetically)
  const popularCountries = POPULAR_COUNTRIES.filter((code) =>
    uniqueCountries.includes(code)
  )
    .map((code) => allCountryObjects.find((c) => c.iso_3166_1 === code))
    .filter(Boolean);

  const otherCountries = allCountryObjects
    .filter((country) => !country.isPopular)
    .sort((a, b) => a.english_name.localeCompare(b.english_name));

  let displayCountries;
  if (showAllCountries) {
    // Show all countries: popular first (in order), then others (alphabetically)
    displayCountries = [...popularCountries, ...otherCountries];
  } else {
    // Show ONLY popular countries available (in watchproviders order)
    displayCountries = popularCountries;
  }

  return {
    countries: displayCountries,
    showToggle,
    popularCount: popularInAvailable.length,
  };
};
