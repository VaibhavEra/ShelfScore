...
export const getLanguageDisplayName = (iso_639_1) => {
  const lang = languageLibrary.find((l) => l.iso_639_1 === iso_639_1);
  if (!lang) return iso_639_1?.toUpperCase() || "Unknown";

  // Return native name if available, otherwise English name
  return lang.name && lang.name.trim() !== "" ? lang.name : lang.english_name;
};

// Helper function to get language for dropdown display (English name + native name)
export const getLanguageDropdownLabel = (iso_639_1) => {
  const lang = languageLibrary.find((l) => l.iso_639_1 === iso_639_1);
  if (!lang) return iso_639_1?.toUpperCase() || "Unknown";

  // Show "English (Native)" format if both exist and are different
  if (lang.name && lang.name.trim() !== "" && lang.name !== lang.english_name) {
